...
<?php
	...
	foreach($logiklan_list as $row):
		//echo ($tanggalTemp!=ambilTanggalSQL($row['insertedon']));
		//if($tanggalTemp!=ambilTangga//lSQL($row['insertedon'])){
	?>
<tr>
		<td><?php echo $i++;?></td>
		<td><?php echo tampilTanggal($row['insertedon'])?></td>
		<td><?php echo tampilJam2($row['insertedon'])?></td>
